...
<head>
    ...
    position: relative;
        ...
        .video-container:hover .delete-icon,
        .video-container:hover .edit-icon {
            visibility: visible; /* Display icons when photo-container is hovered */
        }
...
// Fetch only posts with "for approval" status from the database
$sql = "SELECT * FROM bulletin_files WHERE status = 'for approval' AND is_archived = 0 ORDER BY upload_time DESC";
$result = $db->query($sql);
...
// Add event listener to the checkbox for video files
document.addEventListener('DOMContentLoaded', function() {
    var videoCheckboxes = document.querySelectorAll('.file-checkbox.video');
