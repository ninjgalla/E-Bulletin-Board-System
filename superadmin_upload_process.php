...
function processScheduledUploads($db) {
    ...
    // Query for scheduled uploads with end time not passed
    $sql = "SELECT * FROM bulletin_files WHERE schedule <= ? AND end_time > ?";
    $stmt = $db->prepare($sql);
    ...
    while ($row = $result->fetch_assoc()) {
        // Check if it's time to move to archived
        if ($row['end_time'] <= $currentDateTime) {
            // Move the file to archived directory
            $sourceFilePath = "uploads/" . $row['filename'];
            $destinationFilePath = "archived_uploads/" . $row['filename'];

            // Check if the source file exists
            if (file_exists($sourceFilePath)) {
                // Check if the destination directory exists, if not create it
                if (!is_dir('archived_uploads')) {
                    mkdir('archived_uploads', 0777, true);
                }

                // Attempt to rename (move) the file
                if (rename($sourceFilePath, $destinationFilePath)) {
                    // Update database to reflect that the file has been archived
                    $updateSql = "UPDATE bulletin_files SET archived = 1 WHERE id = ?";
                    ...
                    echo "Error archiving file '{$row['filename']}'.\n";
                }
            } else {
